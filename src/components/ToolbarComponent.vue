<template>
    <v-app-bar elevation="0" app>
        <v-toolbar-title>
            <v-row fluid height="100%" no-gutters class="mr-6" align="center">
                <v-tooltip text="Navigating with Precision and Wisdom" location="bottom">
                    <template v-slot:activator="{ props }">
                        <v-icon class="mr-2" icon="mdi:mdi-source-branch" v-bind="props"></v-icon>
                        <p class="text-h5" v-bind="props">{{ title }}</p>
                    </template>
                </v-tooltip>
                <v-spacer />
                ...
                <v-btn icon>
                    ...
                    <v-menu activator="parent">
                        <v-list>
                            <v-list-item v-for="(item, index) in menuItems" :key="index" @click="item.function">
                                {{ item.title }}
                            </v-list-item>
                        </v-list>
                    </v-menu>
        ...
        <v-dialog v-model="showExitDialog" persistent>
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer />
                    <v-btn rounded @click="showExitDialog = false">Cancel</v-btn>
                    <v-btn rounded @click="exit">Exit</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-app-bar>
