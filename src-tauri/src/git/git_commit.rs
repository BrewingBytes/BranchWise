...
use super::{git_commit_author::GitCommitAuthor, git_user::GitUser};
...
mod tests {
    ...
    fn create_encoded_commit_file(commit_hash: &str, author: GitCommitAuthor, commiter: GitCommitAuthor, tree: Option<&str>, parent_commits: Vec<&str>, message: &str) -> Vec<u8> {
    ...
    fn test_from_string() {
        ...
        let encoded_file_content = create_encoded_commit_file(&commit_hash, commiter.clone(), commiter, Some("50c8353444afbef3172c999ef6cff8d31309ac3e"), Vec::new(), "test commit");

        let git_commit = GitCommit::from_string(commit_hash.clone(), &encoded_file_content).unwrap();
        assert_eq!(git_commit.hash, commit_hash);
        assert_eq!(git_commit.parent_hash, Some(commit_hash));
    }
    ...
    fn test_new() {
        ...
        assert_eq!(git_commit.parent_hash, Some(parent_hash));
